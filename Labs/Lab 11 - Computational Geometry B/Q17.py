...
class KdTree:
    ...
    def points_in_range(self, query_rectangle):
        ...
        """
        if self.is_leaf:
            matches = [p for p in self.points if p.in_box(*query_rectangle)]
        else:
            matches = []
            ...
            if (self.axis == 0 and query_rectangle[1].x >= self.coord) or \
               (self.axis == 1 and query_rectangle[1].y >= self.coord):
                matches += self.rightortop.points_in_range(query_rectangle)
        return matches
    
    def nearest_point(self, p):
        """Returns the nearest point in the tree to p."""
        leaf = self.leaf_containing_point(p)
        closest_in_leaf = self.nearest(p, leaf.points)
        distance = sqrt(closest_in_leaf.lensq())
        bottom_left = Vec(p.x - distance, p.y - distance)
        top_right = Vec(p.x + distance, p.y + distance)
        candidates = self.points_in_range((bottom_left, top_right))
        best = candidates[0]
        for candidate in candidates:
            if (candidate - p).lensq() < (best - p).lensq():
                best = candidate
        return best
        
    def nearest(self, p, points):
        """Returns the nearest point in points to p."""
        closest = points[0]
        for point in points:
            if (point - p).lensq() < (closest - p).lensq():
                closest = point
        return closest
    
    def leaf_containing_point(self, p):
        """Returns the leaf node in the tree rooted at self in which the 
           point p would be placed if inserted into the tree.
        """
        if self.is_leaf:
            return self
        else:
            if (self.axis == 0 and p.x <= self.coord) or \
               (self.axis == 1 and p.y <= self.coord):
                return self.leftorbottom.leaf_containing_point(p)
            else:
                return self.rightortop.leaf_containing_point(p)
...
tree = KdTree(points)
p = Vec(6, 15)
print(f"Nearest point to {p} is {tree.nearest_point(p)}")